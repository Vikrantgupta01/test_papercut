    ...
    public void doWrite(List<InvoicedPrintingJob> printingJobs) {
        ...
        for (InvoicedPrintingJob invoicedPrintingJob: printingJobs){
            ...
            if(job.getColourPages()>0)
                System.out.println("Charges of color pages\t=\t\t\t\t" + invoicedPrintingJob.getColorPrintCost()+currency +"  ("+job.getColourPages() + "X" + invoicedPrintingJob.getPerUnitColorPrint()+")");
            if(job.getMonochromePages()>0)
            System.out.println("Charges of b&w pages\t=\t\t\t\t" + invoicedPrintingJob.getMonoPrintCost() + currency + "  (" + job.getMonochromePages() + "X" + invoicedPrintingJob.getPerUnitMonoPrint() + ")");

            System.out.print("Total Cost\t\t\t\t=\t\t\t\t" + invoicedPrintingJob.getTotalCost() + currency);
            if (job.getColourPages()>0 && job.getMonochromePages()>0){
                System.out.println( "  (" + invoicedPrintingJob.getColorPrintCost() + "+" + invoicedPrintingJob.getMonoPrintCost()+")");
            }
            System.out.println("");
